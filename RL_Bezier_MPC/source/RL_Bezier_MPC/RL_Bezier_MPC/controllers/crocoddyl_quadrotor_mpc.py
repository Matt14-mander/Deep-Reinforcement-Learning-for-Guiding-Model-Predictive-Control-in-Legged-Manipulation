...
class CrocoddylQuadrotorMPC(BaseMPC):
    ...
    def _build_ocp(self):
        ...
        # Create running cost model (must specify nu explicitly)
        ...
        # Terminal cost model (higher weights, nu=0 for terminal)
        terminal_cost_model = crocoddyl.CostModelSum(self.state, 0)

        # Terminal residuals use nu=0 (no control at terminal step)
    ...
    def _update_reference(self, reference_trajectory: np.ndarray):
        ...
        for i, model in enumerate(self.running_models):
            ...
            if hasattr(diff_model, "costs"):
                # Access cost by name directly (Crocoddyl StdMap)
                costs = diff_model.costs.costs
                if "position" in costs:
                    costs["position"].cost.residual.reference = ref_state
        ...
        if hasattr(terminal_diff, "costs"):
            costs = terminal_diff.costs.costs
            if "terminal_position" in costs:
                costs["terminal_position"].cost.residual.reference = terminal_ref_state
...
if CROCODDYL_AVAILABLE:
    ...
    class _QuadrotorDifferentialActionModel(crocoddyl.DifferentialActionModelAbstract):
        ...
        def __init__(self, state, cost_model, mass: float, inertia: np.ndarray, gravity: np.ndarray, nu: int = 4):
        ...
        def calc(self, data, x, u=None):
            ...
            if self.nu == 0:
                # Terminal: no dynamics, just cost
        ...
        def calcDiff(self, data, x, u=None):
            ...
            if self.nu == 0:
                ...
                data.Fx = np.zeros((self.state.nx, self.state.nx))
                data.Fu = np.zeros((self.state.nx, 0))
                data.Lx = data.costs.Lx
                data.Lu = np.zeros(0)
                data.Lxx = data.costs.Lxx
                data.Luu = np.zeros((0, 0))
                data.Lxu = np.zeros((self.state.nx, 0))
                return
