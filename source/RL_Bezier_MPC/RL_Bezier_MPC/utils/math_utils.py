...
# =============================================================================
# Bezier Curve Utilities (for quadruped curve walking)
# =============================================================================
...
def bezier_curve(control_points: np.ndarray, num_samples: int) -> np.ndarray:
    ...
    # Combine: B(t) = b0*P0 + b1*P1 + b2*P2 + b3*P3
    result = (
...
def bezier_tangent(control_points: np.ndarray, t: float) -> np.ndarray:
    ...
    Used by FootholdPlanner to determine heading direction for curve walking.
    ...
    # Derivative control points: Q_i = 3 * (P_{i+1} - P_i)
...
def heading_from_tangent(tangent_xy: np.ndarray) -> float:
    ...
    if np.isscalar(tx):
        ...
        # For near-zero tangents, set heading to 0
