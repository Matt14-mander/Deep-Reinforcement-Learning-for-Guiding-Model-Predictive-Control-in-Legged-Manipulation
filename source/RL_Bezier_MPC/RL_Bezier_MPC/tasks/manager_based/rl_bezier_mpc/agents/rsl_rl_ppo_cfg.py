...
@configclass
class QuadrupedMPCPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    """PPO runner configuration for Quadruped MPC Bezier environment.

    This configuration is tuned for quadruped locomotion with MPC:
    - Larger observation space (45D) and action space (15D)
    - Longer rollouts for gait cycle learning
    - Lower learning rate for stability
    - Higher entropy for gait exploration
    """

    num_steps_per_env = 48  # Even longer rollouts for gait cycles
    max_iterations = 2000  # More iterations for complex locomotion
    save_interval = 100
    ...
    algorithm = RslRlPpoAlgorithmCfg(
        ...
        use_clipped_value_loss=True,
        clip_param=0.2,
        entropy_coef=0.005,  # Lower entropy for more stable gaits
        num_learning_epochs=5,
        num_mini_batches=8,  # More mini-batches for larger batch
        learning_rate=1.0e-4,  # Lower learning rate for stability
        schedule="adaptive",
        gamma=0.99,
        lam=0.95,
        desired_kl=0.008,  # Tighter KL constraint
        max_grad_norm=1.0,
    )